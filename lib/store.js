...
export const useAuthStore = create((set, get) => ({
  ...
  /**
   * Called by /login + /register pages after the API has set the auth cookies.
   * We re-fetch /auth/me to populate the store from the freshly authenticated
   * session (the cookie does the auth — we don't carry tokens in the body).
   */
  acceptSession: async () => {
    try {
